...
class Comment
{
	...
	public function user_has_profile( $author ) {

		global $users;

		// Stop if no User Profiles plugin.
		if ( ! getPlugin( 'User_Profiles' ) ) {
			return false;
		}

		$author = substr( $this->getValue( 'author' ), strlen( 'bludit::' ) );
		$profiles = UPRO_Func\usernames();
		if ( $users->exists( $author ) ) {
			if ( in_array( $author, $profiles ) ) {
				return true;
			}
		}
		return false;
	...
	public function profile_link( $author ) {

		$author = substr( $this->getValue( 'author' ), strlen( 'bludit::' ) );

		if ( $this->user_has_profile( $author ) ) {
			return UPRO_Tags\user_link( $author );
		}
		return false;
	}
